...
local EX_Checks = {
	{ var = "EnableCache", label = "启用玩家缓存", tip = "启用后，观察过的玩家资料会被缓存，你可以稍后再观看。" },
	{ var = "CachePvP", label = "缓存荣誉及竞技场资料", tip = "启用后，除了缓存装备资料外，也一并缓存荣誉及竞技场资料。" },
	{ var = "CombineAdditiveStats", label = "合并计算法术伤害", tip = "启用后，一般法术伤害将会被合计到各类法术伤害中(如火焰、暗影等)。" },
	{ var = "RatingsInPercent", label = "转换属性等级为百分比", tip = "显示属性等级为百分比，以被观察的目标为准" },
	{ var = "ActAsUIFrame", label = "使用标准框架", tip = "启用后，使用标准框架来显示，标准框架和其他内建框架一样，是不可移动的。" },
	{ var = "AutoInspect", label = "切换目标时自动观察", tip = "启用后，会在你切换目标时，重新观察目标。" },
	{ var = "AcceptAddonMsg", label = "启用插件通讯", tip = "启用后，允许别的玩家透过观察助手的通讯机制取得资料。" },
	{ var = "ScanUnknownItems", label = "扫描未知的物品", tip = "启用后，在由缓存区读取物品时，未知的物品会强制读取，这可以多显示一些物品，但可能会导致断线。" },
};
		...
		if (arg3 == "WHISPER") and (arg2 ~= "") and (arg1:sub(1,8) == "EXAMINER") then
			...
			if (data[2] and data[2] == "CMD") then
				...
				elseif (arg2 == "DONE") then
					AzMsg("|2观察助手|r 取得资料 |1"..Examiner_PlayerChatLink(data[1]).."|r，来源: |1"..funcs:PlayerChatLink(arg4).."|r。");
					SendAddonMessage("EXAMINER#"..data[1].."#CMD","RECIEVED","WHISPER",arg4);
					if (cfg.activePage == 2) then
						funcs:BuildCacheList();
					end
				elseif (arg2 == "RECIEVED") then
					AzMsg("|2观察助手|r 传送资料 |1"..Examiner_PlayerChatLink(data[1]).."|r，目的: |1"..funcs:PlayerChatLink(arg4).."|r。");
				end
				return;
			end
			-- Fill out table
...
SlashCmdList[modName] = function(cmd)
	...
	elseif (param1 == "arena") then
		...
		if (type(param2) == "number") then
			AzMsg(format("|2竞技场点数|r |1%d|r 等级 |2=|r 2v2: |1%.1f|r, 3v3: |1%.1f|r, 5v5: |1%.1f|r.",param2,funcs:CalculateArenaPoints(param2,2),funcs:CalculateArenaPoints(param2,3),funcs:CalculateArenaPoints(param2,5)));
		end
	-- Scale
	...
	-- Invalid or No Command
		...
		AzMsg("以下是 |2/examiner|r 或 |2/ex|r 可使用的 |2参数|r:");
		AzMsg(" |2inspect '玩家'|r = 观察玩家 (如不指定则为目前目标)");
		AzMsg(" |2si '物品链接'|r = 扫描物品");
		AzMsg(" |2compare '物品链接1' '物品链接2'|r = 比较物品");
		AzMsg(" |2arena '等级'|r = 竞技场分数计算");
		AzMsg(" |2scale '数值'|r = 设定观察窗口大小 (预设为1)");
		AzMsg(" |2clearcache|r = 清除缓存资料");
	end
end
...
local function ExEvent_ItemSlots_OnEnter(self)
	...
	elseif (self.realLink) then
		...
		GameTooltip:AddLine("物品ID: "..self.realLink:match("item:(%d+)"),0,0.44,0.86);
		GameTooltip:AddLine("这个物品并不安全，你可以点击向服务器查询，\n但可能导致断线。",1,1,1);
		GameTooltip:Show();
	else
		GameTooltip:SetText(_G[self.slotName:upper()]);
	end
...
local function ExEvent_ItemSlots_OnClick(self,button)
	if (self.link) then
		if (button == "RightButton") then
			local gemLink;
			AzMsg("---|2 宝石概观: "..select(2,GetItemInfo(self.link)).." |r---");
			for i = 1, 3 do
				gemLink = select(2,GetItemGem(self.link,i));
				if (gemLink) then
					AzMsg(format("宝石 |1%d|r = %s",i,gemLink));
				end
			end
...
local function ExEvent_ItemSlots_OnUpdate(self,elapsed)
	if (GameTooltip:IsOwned(self)) and (self.link) then
		...
		-- StatTip Hide
		if (not IsAltKeyDown()) and (ex.statTooltip) then
			ex.statTooltip = nil;
			ExEvent_ItemSlots_OnEnter(self);
		-- StatTip Show
...
function funcs:UnitDetailString()
	...
	if (u.race) then
		tinsert(info,u.race ~= "Not specified" and u.race or "未知");
	end
...
function funcs:UnitGuildString()
	...
	if (u.raceFixed) then
		if (u.guild and u.guildRank and u.guildIndex) then
			return u.guildRank.." ("..u.guildIndex..") - <"..u.guild..">";
		end
...
local function FormatTime(time)
	...
	-- under a min
	elseif (time < 60) then
		return color..time.."|r秒";
	-- less than 1 hour
	elseif (time < 60*60) then
		return format(color.."%d|r分"..color.."%.2d|r秒",time/60,mod(time,60));
	-- less than 1 day
	elseif (time < 60*60*24) then
		time = (time/60);
		return format(color.."%d|r时"..color.."%.2d|r分",time/60,mod(time,60));
	-- above 1 day
	else
		time = (time/60/60);
		return format(color.."%d|r天"..color.."%.2d|r时",time/24,mod(time,24));
	end
end
...
local function ExEvent_CacheEntry_OnEnter(self)
	...
	GameTooltip:AddLine(" ");
	GameTooltip:AddDoubleLine("缓存索引:",FauxScrollFrame_GetOffset(ExaminerCacheScroll) + self.id);
	GameTooltip:AddDoubleLine("上次观察:",FormatTime(time()-entry.time));
	GameTooltip:AddLine("Shift+右键: 删除",0.75,0.75,0.75);
	GameTooltip:Show();
end
	...
	for index, slotName in ipairs(ExScanner.Slots) do
		...
		if (button.link) then
			if (GetItemInfo(button.link)) then
				ExScanner:ScanItemLink(button.link,ExStats,ExSets); -- purple
			else
...
function funcs:BuildCacheList()
	...
	-- Update
	ex.cacheHeader:SetText("缓存的玩家 ("..#ExCacheList..")"..(cfg.cacheFilter ~= "" and " |cffffff00*" or ""));
...
function funcs:GetStatValue(statToken,statTable,compareTable,level)
	...
	if (cfg.CombineAdditiveStats) then
		if (statTable["SPELL_DMG"]) then -- purple
			for _, schoolToken in ipairs(EX_MagicSchools) do
				if (statToken == schoolToken.."_SPELL_DMG") then -- purple
					value = (value + statTable["SPELL_DMG"]); -- purple
					break;
				end
			end
		end
		if (statTable["AP"]) and (statToken == "RANGED_AP" or statToken == "FERAL_AP") then -- purple
	...
	if (type(compareTable) ~= "boolean") then
		...
		if (ExScanner.StatRatingBaseTable[statToken]) and (statToken ~= "DEFENSE_RATING") and (statToken ~= "EXPERTISE_RATING") then -- purple
			valuePct = valuePct.."%";
		end
	end
	-- Return
...
function funcs:UpdateResistances()
	local statToken;
	for i = 1, 5 do
		statToken = (EX_MagicSchools[i].."_RES"); -- purple
...
function funcs:BuildStatList()
	...
	if (lastHeader <= #ExStatList) then
--		tinsert(ExStatList,lastHeader,{}); -- purple, no extra line
		tinsert(ExStatList,lastHeader,{ name = "套装" });
	end
	-- Update Resistances
...
function funcs:PVPUpdate()
	if (HasInspectHonorData() or UnitIsUnit(u.token,"player")) then
...
function funcs:UpdateHonor(todayHK, todayHonor, yesterdayHK, yesterdayHonor, lifetimeHK, lifetimeRank, faction)
	...
	if (lifetimeRank ~= 0) then
		...
		ex.pvpHeader:SetText(GetPVPRankInfo(lifetimeRank,u.token).." (等级 "..(lifetimeRank-4)..")");
...
local function ExEvent_CacheIcon_OnEnter(self)
	GameTooltip:SetOwner(self,"ANCHOR_RIGHT");
	GameTooltip:AddLine("缓存的项目");
	GameTooltip:AddDoubleLine("地区:",u.zone,1,1,1);
	GameTooltip:AddDoubleLine("过时:",FormatTime(time() - u.time),1,1,1);
	GameTooltip:AddDoubleLine("日期:",date("%Y年 %m月 %d日",u.time),1,1,1);
	GameTooltip:AddDoubleLine("时间:",date("%H:%M:%S",u.time),1,1,1);
	GameTooltip:Show();
end
...
local function ExEvent_InfoIcon_OnEnter(self)
	...
	-- Generate Tooltip
	GameTooltip:SetOwner(self,"ANCHOR_RIGHT");
	GameTooltip:AddLine("其他");
--	GameTooltip:AddDoubleLine("单位代码",tostring(u.token),1,1,1);
	if (u.talents) then
		GameTooltip:AddDoubleLine("天赋专精",u.talents,1,1,1);
	end
	GameTooltip:AddDoubleLine("合并物品等级",iLvlTotal,1,1,1);
	GameTooltip:AddDoubleLine("平均物品等级",format("%.2f",iLvlTotal / (#ExScanner.Slots - 2)),1,1,1);
	GameTooltip:AddDoubleLine("宝石数量",gemCount,1,1,1);
	GameTooltip:AddDoubleLine("宝石颜色",format("|cffff6060%d|r/|cffffff00%d|r/|cff6060ff%d",gemRed,gemYellow,gemBlue),1,1,1);
	GameTooltip:Show();
end
...
local function ExEvent_Buttons_OnClick(self,button)
	...
	if (button == "LeftButton") then
		...
		elseif (id == 2) and (IsShiftKeyDown()) then
			AzDialog:New("输入新的过滤条件...",cfg.cacheFilter,function(text) cfg.cacheFilter = text; funcs:BuildCacheList(); end);
...
local function DropDown_SelectItem()
	...
	elseif (this.value == 1) then
		AzDialog:New("新增过滤条件...",cfg.cacheFilter,function(text) cfg.cacheFilter = text; funcs:BuildCacheList(); end);
	-- Clear Cache
	elseif (this.value == 2) then
		local entryCount = 0;
		table.foreach(cache,function() entryCount = (entryCount +1); end);
		AzDialog:New("你确定要清除 "..entryCount.." 项资料吗?",nil,function() Examiner_Cache = {}; cache = Examiner_Cache; funcs:BuildCacheList(); end);
	-- Cache Player
	...
	elseif (this.value == 12) then
		...
		AzDialog:New("输入要传送资料的对象...",name,funcs.SendPlayer);
...
local EX_SortMethods_Text = { "名称", "服务器", "等级", "公会", "种族", "职业", "时间" };
function funcs:DropDown_Initialize()
	...
	if (this.id == 2) then
		...
		-- method
		for i, method in ipairs(EX_SortMethods) do
			info.text = "以"..EX_SortMethods_Text[i].."排序";
			info.value = method;
...
function funcs:SendPlayer(target)
	...
	if (not ex.statsLoaded) then
		AzMsg("|2观察助手|r 无观察资料可传送!");
		return;
	end
	-- Check if we have someone to send to
	if (not target or target == "") then
		AzMsg("|2观察助手|r 需指定传送对象");
		return;
	end
	...
	AzMsg("|2观察助手|r 玩家资料 |1"..funcs:PlayerChatLink(entryName).."|r 已发送给 |1"..funcs:PlayerChatLink(target).."|r。");
end
...
do
	...
	local function SetHeaderFont(fontstring)
		fontstring:SetFont("Fonts\\ZYKai_T.TTF",16,"THICKOUTLINE");
...
--	ex:RegisterEvent("SHOW_COMPARE_TOOLTIP");
	...
	ex.buttons = {};
	local EX_Buttons = {
		{ label = "设定", header = "观察助手设定", tip = "观察助手设定" },
		{ label = "缓存", header = "缓存的玩家", tip = "右键可使用菜单" },
		{ label = "统计", header = "装备属性统计", tip = "右键可使用菜单" },
		{ label = "PvP", header = "玩家对战", tip = "荣誉及竞技场信息" },
		{ label = "天赋", header = "显示天赋", tip = "天赋专精显示" },
	};
	...
	for index, slot in ipairs(ExScanner.Slots) do
		...
		t.border:SetTexture("Interface\\Addons\\ExaminerX\\Textures\\Border");
	...
	t:SetText("观察助手 |cffffff80"..GetAddOnMetadata(modName,"Version"));
	...
	for i = 1, 5 do
		...
		t.value:SetFont("Fonts\\ZYKai_T.TTF",12,"OUTLINE");
	...
	for i = 1, 9 do
		...
		if (i <= 3) then
			t:SetText(i == 1 and "今天" or i == 2 and "昨天" or "累计");
...
--		t.details[#t.details - 1]:SetWidth(130);
		t.details[#t.details]:SetWidth(90);
