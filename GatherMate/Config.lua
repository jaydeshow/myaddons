...
options.args.importing.args.GatherMateData = {
	...
	args = {
		...
		loadBC = {
			order = 4,
			name = L["BC Data Only"],
			desc = L["Only import Burning Crusade data from WoWHead"],
			type = "toggle",
			get  = "GetBCOnly",
			set  = "SetBCOnly",
			arg  = "GatherMate_Data",
		...
		loadData = {
			...
			func = function() 
				...
				if loaded then
					...
					GatherMateData:PerformMerge(db.importers["GatherMate_Data"].Databases,db.importers["GatherMate_Data"].Style,db.importers["GatherMate_Data"].bcOnly)
	...
	for k,v in pairs(db.importers) do
		...
		if verline and v["autoImport"] then 
			...
			if dataVersion and dataVersion > v["lastImport"] then
				...
				if loaded then
					addon:PerformMerge(v.Databases,v.Style,v.bcOnly)
