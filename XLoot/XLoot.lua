...
local function IsSwift()
	return GetAutoLootDefault() ~= IsModifiedClick("AUTOLOOTTOGGLE")
end
...
function XLoot:AddLootFrame(id)
	...
	button:SetScript("OnEnter", 	function() local slot = this:GetID() if LootSlotIsItem(slot) then GameTooltip:SetOwner(this, "ANCHOR_RIGHT") GameTooltip:SetLootItem(slot) if IsShiftKeyDown() then GameTooltip_ShowCompareItem() end CursorUpdate() end end )
	button:SetScript("OnUpdate", function() CursorOnUpdate() end)
...
function XLoot:SetupFrames()
   ...
   self.frame:SetScale(self.db.profile.scale)
	...
	self.closebutton:SetScript("OnClick", function() XLoot:AutoClose(true, true); end)
