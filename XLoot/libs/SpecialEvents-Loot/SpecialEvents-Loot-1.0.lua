...
local vmajor, vminor = "SpecialEvents-Loot-1.0", "$Revision: 54340 $"
...
-- Local fun.
local activerolls = 0
local lootpatterns, rollhandlers, unsortedloot, currentsort

-- Chatmsg handler
local function Handler(text)
	-- Handle group loot mode only if we're in the mode and there's a roll going
	if GetLootMethod() == 'group' and activerolls > 0 then
