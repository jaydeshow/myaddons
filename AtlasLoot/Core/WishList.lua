...
function AtlasLoot_AddToWishlist(itemID, itemTexture, itemName, lootPage, sourcePage)
	for _, v in ipairs(AtlasLootCharDB["WishList"]) do
		if v[1] == itemID then
			DEFAULT_CHAT_FRAME:AddMessage(BLUE..AL["AtlasLoot"]..": "..itemName..RED..AL[" already in the WishList!"]);
			return;
		end
	end
	table.insert(AtlasLootCharDB["WishList"], { itemID, itemTexture, itemName, lootPage, sourcePage });
	DEFAULT_CHAT_FRAME:AddMessage(RED..AL["AtlasLoot"]..": "..itemName..GREY..AL[" added to the WishList."]);
...
function AtlasLoot_DeleteFromWishList(itemID)
	...
	for i, v in ipairs(AtlasLootCharDB["WishList"]) do
		if v[1] == itemID then
			DEFAULT_CHAT_FRAME:AddMessage(RED..AL["AtlasLoot"]..": "..v[3]..GREY..AL[" deleted from the WishList."]);
...
function AtlasLoot_CategorizeWishList(wlTable)
	...
	for k, v in pairs(categories) do
		...
		if #result > 1 and #result % 15 > 0 then table.insert(result, { 0, "", "", "" }) end
		-- If a subheading is on the last row of a column, push it to next column
		if (#result + 1) % 15 == 0 then table.insert(result, { 0, "", "", "" }) end
		-- Subheading
