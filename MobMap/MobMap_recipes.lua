...
function MobMap_GetRecipeDetails(recipeID)
	...
	local minCount=MobMap_Mask(data1/mobmap_poweroftwo[18],mobmap_poweroftwo[10]);
	local maxCount=MobMap_Mask(data1/mobmap_poweroftwo[28],mobmap_poweroftwo[10]);
	local minLevel=MobMap_Mask(data1/mobmap_poweroftwo[38],mobmap_poweroftwo[10]);
	...
	local recipeItem=MobMap_Mask(data2/mobmap_poweroftwo[22],mobmap_poweroftwo[16]);
	local sourceType=MobMap_Mask(data2/mobmap_poweroftwo[38],mobmap_poweroftwo[2]);
	...
	for reagent=1,reagentCount,1 do
		...
		currentReagent["itemid"]=MobMap_Mask(data3/mobmap_poweroftwo[26*(currentReagentPointer%2)],mobmap_poweroftwo[16]);
		currentReagent["dropinfo"]=MobMap_Mask(data3/mobmap_poweroftwo[26*(currentReagentPointer%2)+16],mobmap_poweroftwo[1]);
		if(currentReagent["dropinfo"]==1) then 
			currentReagent["dropinfo"]=true;
		else 
			currentReagent["dropinfo"]=false;
		end
		currentReagent["buyable"]=MobMap_Mask(data3/mobmap_poweroftwo[26*(currentReagentPointer%2)+17],mobmap_poweroftwo[1]);
		if(currentReagent["buyable"]==1) then
			currentReagent["buyable"]=true;
		else
			currentReagent["buyable"]=false;
		end
		currentReagent["count"]=MobMap_Mask(data3/mobmap_poweroftwo[26*(currentReagentPointer%2)+18],mobmap_poweroftwo[8]);
		table.insert(reagents,currentReagent);
	end

	return productID, isEnchant, minCount, maxCount, minLevel, reagents, recipeItem, sourceType;
end
...
function MobMap_RecipeListProfessionFilter_OnLoad()
	UIDropDownMenu_Initialize(MobMapRecipeListProfessionFilter, MobMap_RecipeListProfessionFilter_Initialize);
	UIDropDownMenu_SetWidth(180);
	UIDropDownMenu_SetSelectedID(MobMapRecipeListProfessionFilter, 1);
end
...
function MobMap_RecipeListProfessionFilter_OnClick()
	UIDropDownMenu_SetSelectedID(MobMapRecipeListProfessionFilter, this:GetID());
...
function MobMap_ShowRecipeDetails(recipeid)
	if(recipeid==nil) then
		...
		local productID, isEnchant, minCount, maxCount, minLevel, reagents, recipeItem, sourceType = MobMap_GetRecipeDetails(recipeid);
