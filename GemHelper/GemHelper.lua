--[[****************************************************************************************
	...
	****************************************************************************************
	Description:
		...
		where the gem can be found/purchased. Currently, the database contains 218 gems.
...
local function GemHelper_Init()
	...
	for k, v in ipairs(GemHelper_CheckButtonIndexes) do
		...
		checkButtonText = _G["GemHelper_CheckButton"..v.."Text"]
	...
	for v = 1, 3 do
		...
		checkButton:SetID(v)
		checkButton:SetPoint("TOPRIGHT", _G["GemHelper_Background"..v], "TOPRIGHT", -3, -3)
		if GemHelper_Save[-v] == nil then
			-- SavedVariable Defaults
			GemHelper_Save[-v] = false
		end
		...
		RaiseFrameLevel(checkButton)
		checkButton:SetID(v)
		checkButton:SetPoint("TOPRIGHT", _G["GemHelper_CollapseButton"..v], "TOPLEFT", 5, 0)
	end
	...
	GemHelper_CheckButton70:SetPoint("TOPLEFT", GemHelper_CheckButton3, "TOPLEFT", 325, 0)
	...
	for v = 1, GEMHELPER_ITEMS_SHOWN do
		...
		if v == 1 then
			...
			checkButton:SetPoint("TOPLEFT", _G["GemHelper_ItemButton"..v-1], "BOTTOMLEFT", 0, 0)
		end
	end
	...
	for _, v in ipairs(GemHelper_GCheckButtonIndexes[2]) do
		checkButton = _G["GemHelper_CheckButton"..v]
	...
	for buttonID = 1, 3 do
		if GemHelper_Save[-buttonID] then
			...
			for k, v in ipairs(GemHelper_GCheckButtonIndexes[buttonID]) do
				_G["GemHelper_CheckButton"..v]:Hide()
			end
			checkButton = _G["GemHelper_CollapseButton"..buttonID]
			...
			_G["GemHelper_Background"..buttonID]:SetHeight(GemHelper_CollapseHeight[buttonID][2])
			if buttonID < 3 then
				_G["GemHelper_CheckButton"..(buttonID+1)]:SetPoint("TOPLEFT", _G["GemHelper_CheckButton"..buttonID], "TOPLEFT", 0, -GemHelper_CollapseHeight[buttonID][2])
			else
			...
			-- Uncollapse
			for k, v in ipairs(GemHelper_GCheckButtonIndexes[buttonID]) do
				_G["GemHelper_CheckButton"..v]:Show()
			end
			checkButton = _G["GemHelper_CollapseButton"..buttonID]
			...
			_G["GemHelper_Background"..buttonID]:SetHeight(GemHelper_CollapseHeight[buttonID][1])
			if buttonID < 3 then
				_G["GemHelper_CheckButton"..(buttonID+1)]:SetPoint("TOPLEFT", _G["GemHelper_CheckButton"..buttonID], "TOPLEFT", 0, -GemHelper_CollapseHeight[buttonID][1])
			else
	...
	GemHelper_UserDropdown:SetPoint("LEFT", GemHelper_CheckButton1012, "LEFT", -3, -6)
	GemHelper_CheckButton1012Text:SetPoint("LEFT", GemHelper_CheckButton1012, "RIGHT", 17, 0)

	-- Add the slash command
	SlashCmdList["GEMHELPER"] = function(msg)
		if GemHelper_Frame:IsVisible() then
	...
	-- This is to reduce the number of times GemHelper updates inventory changes
...
function GemHelper_Filter()
	...
	if GemHelper_Save[101] and GemHelper_Save[1011] then
		local _, bagNum_Slots, itemLink, itemID
	...
	for gemID, gemTable in pairs(GemHelper_GemData) do
		...
		if GemHelper_Save[1] then
			flag1 = false
			local t = GemHelper_GCheckButtonIndexes[1]
			for i = 1, #t do
				local j = t[i]
				if GemHelper_Save[j] and gemTable[j] then
					flag1 = true
					break
				end
			end
		...
		-- Filter by Material
		flag2 = true
		if flag1 and GemHelper_Save[2] then
			flag2 = false
			local t = GemHelper_GCheckButtonIndexes[2]
			for i = 1, #t do
				local j = t[i]
				if GemHelper_Save[j] and gemTable[j] then
					flag2 = true
		...
		-- Filter by Statistic
		flag3 = true
		if flag1 and flag2 and GemHelper_Save[3] then
			flag3 = false
			local t = GemHelper_GCheckButtonIndexes[3]
			for i = 1, #t do
				local j = t[i]
				if GemHelper_Save[j] and gemTable[j] then
					flag3 = true
		...
		if flag1 and flag2 and flag3 then
			flag4 = false
			local t = GemHelper_GCheckButtonIndexes[4]
			for i = 1, #t do
				local j = t[i]
				if GemHelper_Save[j] and gemTable[j] then
					flag4 = true
		...
		flag5 = true
		if flag1 and flag2 and flag3 and flag4 and GemHelper_Save[101] and GemHelper_Save[1011] then
			flag5 = false
			local t = GemHelper_GCheckButtonIndexes[2]
			for i = 1, #t do
				local j = t[i]
				if bagScan[j] and gemTable[j] then
					flag5 = true
					break
				end
			end
		...
		flag6 = true
		if flag1 and flag2 and flag3 and flag4 and flag5 and GemHelper_Save[101] and GemHelper_Save[1012] then
			flag6 = false
...
function GemHelper_UpdateCheckboxes(buttonID)
	for k, v in ipairs(GemHelper_GCheckButtonIndexes[buttonID]) do
		if GemHelper_Save[buttonID] then
			--OptionsFrame_EnableCheckBox(_G["GemHelper_CheckButton"..v])
			GemHelper_EnableCheckBox(_G["GemHelper_CheckButton"..v])
		else
			--OptionsFrame_DisableCheckBox(_G["GemHelper_CheckButton"..v])
			GemHelper_DisableCheckBox(_G["GemHelper_CheckButton"..v])
		end
	end
...
function GemHelper_ResetButton_OnClick()
	...
	for k, v in ipairs(GemHelper_GCheckButtonIndexes[buttonID]) do
		...
		_G["GemHelper_CheckButton"..v]:SetChecked(GemHelper_Save[v])
	end
...
function GemHelper_Update()
	...
	for i = 1, GEMHELPER_ITEMS_SHOWN do
		...
		button = _G["GemHelper_ItemButton"..i]
		...
		if itemIndex <= #GemHelper_DispResults then
			...
			if itemID == selectedGemID then
				...
				_G["GemHelper_ItemButton"..i.."Highlight"]:Show()
			else
				button:UnlockHighlight()
				_G["GemHelper_ItemButton"..i.."Highlight"]:Hide()
			end
			button:Show()
		else
			button:Hide()
