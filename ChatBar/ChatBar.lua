...
function ChatBar_PrintSlideInfo()
	ChatBar_Message("CLIENT",ChatBarFrame.isSliding, ChatBarFrame.endsize, ChatBarFrame.velocity);
end
...
function ChatBar_ReorderChannels()
	if UnitOnTaxi("player") then
		ChatBar_Message("CLIENT", ChatTypeInfo["SYSTEM"], CHATBAR_REORDER_FLIGHT_FAIL);
		...
		if (type(chanName) == "string") then
			...
			if (STANDARD_CHANNEL_ORDER[simpleName]) then
				if ( simpleName == "GlobalComm" ) then 
					...
					tempChannel[STANDARD_CHANNEL_ORDER[simpleName]] = simpleName;
				end
	...
	ChatBar_Message("CLIENT",CHATBAR_REORDER_START);
