...
function TBTFrame_OnLoad()
	...
	hooksecurefunc("TradeFrame_OnShow", function()if(UnitExists("NPC"))then TradeFrameTargetInfoText:SetText(UnitClass("NPC").." - "..UnitLevel("NPC"));end end);
	...
	button:SetScript("OnClick", function() DoEmote("hungry", "NPC") end);
	...
	button:SetScript("OnClick", function() DoEmote("thank", "NPC") end);
	...
	hooksecurefunc("ContainerFrameItemButton_OnModifiedClick", function(button)
		...
		if(button == "LeftButton" and IsShiftKeyDown() and AuctionFrame and AuctionFrame:IsVisible()) then
			...
			ChatEdit_InsertLink(GetContainerItemLink(this:GetParent():GetID(), this:GetID()));
...
function TBT_TradeItem(type)
	...
	if(maxRank) then
		...
		TBTFrame_SetButtonSpell(this, spell);
	end
end
...
function TBTFrame_CreateLeftButton(class)
	...
	if(class=="MAGE") then
		...
		button:SetScript("PreClick", function() TBT_TradeItem("water") end);
		button:SetScript("PostClick", function() TBTFrame_SetButtonSpell(this,"") end);
		...
		button:SetScript("PreClick", function() TBT_TradeItem("food") end);
		button:SetScript("PostClick", function() TBTFrame_SetButtonSpell(this,"") end);
	elseif (class=="WARLOCK") then
		...
		button:SetPoint("TOPLEFT", "TradeFrame", "TOPLEFT", 72, -50);
		button:SetScript("PreClick", function() TBT_TradeItem("stone") end);
		button:SetScript("PostClick", function() TBTFrame_SetButtonSpell(this,"") end);
	elseif (class=="ROGUE") then
		...
		button:SetScript("PostClick", function() ClickTargetTradeButton(7); end);
	end
end

TBT_MaxSpellRank = nil;

function TBTFrame_OnEvent()
