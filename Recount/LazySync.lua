		...
		for i = 1, GetNumRaidMembers(), 1 do 
			...
			if not UnitIsVisible(unitid) and UnitExists(unitid) then
				...
				if not combatant then
					...
					combatant = Recount.db2.combatants[name]
				end
			
				Recount.db2.combatants[name].lazysync = true
				Recount.lazysync = Recount.lazysync or Recount:CheckRetention(name)
			end
		end
...
function Recount:SendSelf(target)
	...
	local combatant = Recount.db2.combatants[myname]
...
function Recount:BroadcastLazySync()
	...
	if not Recount.lazysync and not Recount.Debug then return end -- Elsia: Nothing to lazy sync
	...
	if data and data.Pet then
		for i=1,#data.Pet do
			--local petrecord = Recount.db2.combatants[data.Pet[i]] and Recount.db2.combatants[data.Pet[i]].Sync
			local petname = data.Pet[i]
			if Recount:GetLazySyncTouched(myname,petname) then
				local damage = Recount:GetLazySyncAmount(myname,petname,"Damage") or 0
				local damagetaken = Recount:GetLazySyncAmount(myname,petname,"DamageTaken") or 0
	...
	local plevel = Recount.db2.combatants[Recount.PlayerName].level
	...
	if Recount.Debug then
		local name = Recount.PlayerName
		local combatant = Recount.db2.combatants[name]
		if combatant and combatant.lazysync or Recount.Debug then
			-- Sync self
			if serialdata then
			...
			elseif cmd == "PS" then -- Player data set (when first meeting up)
				...
				if type(name)~="number" and (not Recount.VerNum[owner] or Recount.VerNum[owner]>=MinimumV) then
					...
					if Recount:CheckRetention(name) then
						...
						Recount:DPrint("PS WITHOUT retention: "..name)
					end
				end
		...
		if worked == true then
			if cmd == "RS" then -- Reset broadcast
				...
				if Recount:CheckRetention(name) then
					...
					Recount:DPrint("Received RS and NOT retaining: "..name)
				end
...
function Recount:ConfigComm()
	...
	if Recount.Debug and GetNumPartyMembers() == 0 and GetNumRaidMembers() ==0 then
...
function Recount:FlagSync()
	if GetNumRaidMembers() > 0 then
		for i = 1, GetNumRaidMembers(), 1 do 
			if UnitExists("raid"..i) then
				Recount.db2.combatants[UnitName("raid"..i)].lazysync = true
	...
	elseif GetNumPartyMembers() > 0 then
		for i = 1, GetNumPartyMembers(), 1 do 
			if UnitExists("party"..i) then
				Recount.db2.combatants[UnitName("party"..i)].lazysync = true
				Recount.lazysync = true
			end
		end
	end
