...
local function DropDownMember(name, dkp)
	local id = MerDKP.tmp.id
	if not id or not MerDKP.db[id] then return end
	if this.value == "WHISPER" then
		SendChatMessage(MerDKP.opt.prefix.."<"..MerDKP.db[id].title.."> "..name.."："..dkp.." DKP", "WHISPER", nil, name)
	else
...
function DropDownChannel()
	local id = this:GetID()
	UIDropDownMenu_SetSelectedID(MerDKPFrameListDropDownChannal, id)
...
M.opt = {
	...
	Outputmax 	= 25, --一次性发送的最多条数
	Minimap 	= 360, --小地图图标位置 0-隐藏
}

M.tmp = {
	id 			= nil, --GUI显示的DKP id
	DkpMin 		= -5000, --显示的最小dkp分数,太低的就不显示
	total 		= 0, --列表总数
	channel 	= "RAID", --发送的频道
...
M.tmp.tbc = {
	{text = "布甲", "牧师","法师","术士",UNKNOWN,},
	{text = "皮甲", "潜行者","德鲁伊",UNKNOWN,},
	{text = "锁甲", "猎人","萨满祭司",UNKNOWN,},
	{text = "板甲", "战士","圣骑士",UNKNOWN,},
	{text = "所有职业", "圣骑士","战士","牧师","法师","术士","德鲁伊","潜行者","猎人","萨满祭司",UNKNOWN,},
}
M.tmp.classes = {
...
M.DROPMENU = {
	...
	member = {
		...
		{ text = "修改", value = "CLASS", func = function(name,dkp) MerDKP_OpenPopupFrame("edit", MerDKP.tmp.id, name, this.value, dkp) end, },
		{ text = "删除", value = "CLASS", func = function(name,dkp) MerDKP_OpenPopupFrame("delete", MerDKP.tmp.id, name, this.value, dkp, "确定删除?") end, },
	...
	slider = {
		...
		{ text = "设置最大发送数目", Min = 1, Max = 100, Step = 1, key = "Outputmax",
		  tooltipText = "设置一次发送的最大数目,以前发送太多断线",
...
function M:OnVarsLoaded()
	...
	ChatFrame_OnEvent = function(event)
		...
		if self.opt.HideWhisper and event == "CHAT_MSG_WHISPER_INFORM" and strfind(arg1,"^"..MerDKP.opt.prefix) then
			return
		end
		MerDKP_ChatFrame_OnEvent(event)
	end
...
function M:CHAT_MSG_WHISPER(arg1, arg2)
	...
	if arg1 == self.opt.rplist then
		...
		SendChatMessage(self.opt.prefix..MerDKP.opt.mydkp.." --查询个人所有DKP", "WHISPER", nil, arg2)
		SendChatMessage(self.opt.prefix..MerDKP.opt.rp.."+数字 职业 --查询该职业DKP", "WHISPER", nil, arg2)
		SendChatMessage(self.opt.prefix..MerDKP.opt.rp.."+数字 名字 --查询该人物DKP", "WHISPER", nil, arg2)
