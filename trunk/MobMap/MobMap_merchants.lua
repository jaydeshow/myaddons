	...
	for merchantid=1,MobMap_GetMerchantCount(),1 do
		...
		if(npcid~=nil) then
			...
			if(match==true and zoneid~=nil) then
				if(MobMap_GetMobZone(npcid)~=zoneid) then match=false; end
			end
...
function MobMap_GetDetailsForMerchant(merchantid)
	...
	local itemcount=MobMap_Mask(data/mobmap_poweroftwo[36],mobmap_poweroftwo[7]);
	local isExtended=MobMap_Mask(data/mobmap_poweroftwo[43],mobmap_poweroftwo[1]);
	if(isExtended==0) then
		isExtended=false;
	else
		isExtended=true;
	end
	local tokencount=MobMap_Mask(data/mobmap_poweroftwo[44],mobmap_poweroftwo[3]);
...
function MobMap_UpdateMerchantEntry(entry, merchantid)
	...
	if(merchantid==nil) then
		...
		local zoneid=MobMap_GetMobZone(npcid);
		if(zoneid~=nil) then
			...
			if(itemdata[item].tokens~=nil and table.getn(itemdata[item].tokens)>0) then
				for token=1,table.getn(itemdata[item].tokens),1 do
					...
					elseif(token==1 and(itemdata[item].arenaprice==0 and itemdata[item].honorprice==0)) then
						tokenframe:SetPoint("LEFT", frame_altcurrency:GetName().."Points", "LEFT", 0, 0);	
					else
...
function MobMap_MerchantItem_OnClick()
	if(IsControlKeyDown()) then
		if(this:GetParent().itemlink~=nil) then
