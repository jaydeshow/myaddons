...
local MINOR_VERSION = tonumber(("$Revision: 66480 $"):match("(%d+)")) - 60000
...
do
	...
	do
		...
		local function GetCaretPosition(this)
			local text = this:GetText()
			if #text == 0 then
				return 0
			end
			local OnTextChanged = this:GetScript("OnTextChanged")
		...
		local function SetCaretPosition(this, position)
			local text = this:GetText()
			if #text == 0 then
				return
			end
		...
		local function editBox_OnTextChanged(this, ...)
			...
			local parent = this:GetParent()
			local text = (this:GetText() or ''):gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
			local tmp = newList(text)
