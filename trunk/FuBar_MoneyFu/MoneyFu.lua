...
MoneyFu = AceLibrary("AceAddon-2.0"):new("FuBarPlugin-2.0", "AceDB-2.0", "AceEvent-2.0", "AceHook-2.1", "AceConsole-2.0")
MoneyFu.hideWithoutStandby = true
...
function MoneyFu:OnMenuRequest(level, value)
	if level == 1 then
		...
		dewdrop:AddLine(
			'text', L["SHOW_PER_HOUR_CASHFLOW"],
			'arg1', self,
			'func', function()
				self.db.profile.showPerHour = not self.db.profile.showPerHour
				self:UpdateTooltip()
			end,
			'checked', self.db.profile.showPerHour
		)
