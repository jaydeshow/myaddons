...
function AtlasLootItem_OnClick(arg1)
    ...
    elseif(IsAltKeyDown() and (this.itemID ~= 0)) then
        if AtlasLootItemsFrame.refresh[1] == "WishList" then
            ...
            AtlasLoot_AddToWishlist(this.itemID, this.itemTexture, getglobal("AtlasLootItem_"..this:GetID().."_Name"):GetText(), "=ds="..AtlasLootItemsFrame.refresh[3], AtlasLootItemsFrame.refresh[1].."|"..AtlasLootItemsFrame.refresh[2]);
	    end
	elseif(AtlasLootItemsFrame.refresh[1] == "WishList" and this.sourcePage) then
		local dataID, dataSource = strsplit("|", this.sourcePage);
		if(dataID and dataSource and AtlasLoot_IsLootTableAvailable(dataID)) then
			AtlasLoot_ShowItemsFrame(dataID, dataSource, getglobal("AtlasLootItem_"..this:GetID().."_Extra"):GetText(), AtlasLootItemsFrame.refresh[4]);
		end
    end
end
