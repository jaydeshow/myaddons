afp_SearchItems = {"源生", "位面", "棱光", "印记", "日怒徽记", "火翼徽记", "莲花", "灵网", "邪能", "奥法宝典"};
aft_MAX_SEARCH_ITEMS = 20;
...
v20100-2
-- Several updates and fixes added (courtesy Drizzd)
>> Reset button removed (part of default Blizzard UI now)
>> Outdated calls removed
>> Tooltip slowdown code merged

v20100-1
-- Updated for The Burning Crusade.
-- Added Planar Essence and Netherweave Bolts.
-- Removed AuctionSort functions; please use AuctionSort alongside this mod if you wish that functionality.
	...
	--if (afp_OptionButton_ExactName:GetChecked() and BrowseName and BrowseName:GetText() ~= "") then
		for i=1, NUM_BROWSE_TO_DISPLAY do
			--for n=1,NUM_AUCTION_ITEMS_PER_PAGE do
				...
				if (name and BrowseName and string.lower(name) == string.lower(BrowseName:GetText())) or BrowseName:GetText() == "" or not afp_OptionButton_ExactName:GetChecked() then
					...
					elseif (name==AFP_ITEM_BOLT_RUNECLOTH) then
						count = count * 5;
					elseif (name==AFP_ITEM_BOLT_NETHERWEAVE) then
						count = count * 6;
					...
					elseif (name==AFP_ITEM_ESSENCE_MAGIC) then
						count = count * 3;
					elseif (name==AFP_ITEM_ESSENCE_PLANAR) then
						count = count * 3;
...
function afp_FlyoutButton_OnClick()
	if (afp_FlyoutButton and afp_FlyoutButton:IsVisible()) then
		...
		elseif (afp_FlyoutFrame) then
			...
			afp_FlyoutSetButton:SetParent(afp_FlyoutFrame);
			afp_FlyoutSetButton:SetPoint("TOPRIGHT","afp_FlyoutClear","BOTTOMRIGHT",0,0);
			afp_FlyoutSetButton:Show();
...
function afp_SlashHandlerSetButton(msg)
	if (not msg or msg=="") then
		afp_Print("请输入逗号分开的字符串, 例如/setbutton 黑莲花,魔莲花");
	else
		msg = string.gsub(msg,"，",",");
		afp_SearchItems = {string.split(",", msg)};
		afp_UpdateSearchButtons();
	end
end
...
function afp_UpdateSearchButtons()
	afp_FlyoutFrame:SetHeight(AFP_LINES*19 + 5 + table.getn(afp_SearchItems)*22);
	for i=1, aft_MAX_SEARCH_ITEMS do
		local button = getglobal("afp_SearchButton"..i);
		if(afp_SearchItems[i]) then
			button:SetText(afp_SearchItems[i]);
			button:Show();
		else
			button:Hide();
		end
