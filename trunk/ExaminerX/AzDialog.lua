...
AzDialog:SetBackdrop({ bgFile="Interface\\ChatFrame\\ChatFrameBackground", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", tile = true, tileSize = 8,edgeSize = 12, insets = { left = 2, right = 2, top = 2, bottom = 2 } });
...
AzDialog:SetScript("OnMouseDown",function(self) self:StartMoving(); end);
AzDialog:SetScript("OnMouseUp",function(self) self:StopMovingOrSizing(); end);
...
AzDialogEdit:SetScript("OnEnterPressed",function(self) self:GetParent():OK(); end);
AzDialogEdit:SetScript("OnEscapePressed",function(self) self:GetParent():Cancel(); end);

-- Cancel
CreateFrame("Button","AzDialogCancel",AzDialog,"UIPanelButtonTemplate");
AzDialogCancel:SetWidth(75);
AzDialogCancel:SetHeight(21);
AzDialogCancel:SetScript("OnClick",function(self) self:GetParent():Cancel(); end);
AzDialogCancel:SetText("取消");
...
AzDialogOK:SetScript("OnClick",function(self) self:GetParent():OK(); end);
AzDialogOK:SetText("确定");
...
function AzDialog:New(header,text,funcOK,funcCancel)
	...
	-- Set Type
	AzDialogHeader:SetText(header or "输入文字...");
	if (text) then
		AzDialog:SetHeight(88);
